...
**MVP amendment (2026-05-21):** Posts, Engagement, and Feed shipped consolidated into a single `TrueCapture.Modules.Social` project (alongside the follow graph, stories, and in-app notifications) rather than the three separate `Modules.Posts` / `Modules.Engagement` / `Modules.Feed` projects originally specified. Reason: they share the `Post` aggregate and engagement counters so tightly that three projects produced only cross-project plumbing at MVP scale. The module boundary toward *other* modules (Identity, Users, Media) is still enforced; the internal Posts/Engagement/Feed seam is kept as namespaces (`Entities/`, `Services/`) inside `Modules.Social` so the project can still be split along those lines when feed scale demands it. The ArchUnit boundary test (task 1.3) treats `Modules.Social` as one module.
